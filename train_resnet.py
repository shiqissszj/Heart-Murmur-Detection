...
from HumBugDB.runTorchMultiClass import LogisticRegression
...
def create_model(model_name, num_classes):
    if model_name == "resent50":
        ...
        # training = train_model_LogisticRegression
...
def run_model_training(
    ...
    if classes_name == "pitchs":
        y_train = s_pitchs_train.to(device)
        y_test = s_pitchs_test.to(device)
        model_s, training_s = create_model(model_name, 5)  # 4
        training_s(
            X_train,
            y_train,
            clas_weight=weights,
    ...
    elif classes_name == "murmur":

        y_train = murmurs_train.to(device)
        y_test = murmurs_test.to(device)
        model_d, training_d = create_model(model_name, 3)  # 4
        training_d(
            X_train,
            y_train,
            clas_weight=weights,
    ...
    elif classes_name == "LR":

        X_train = calculate_pitchs_output(
        recalc_output,
        train_data_directory,
        dbres_output_directory,
        "models/model_Pitchs.pt",
        )
        
        X_test = calculate_pitchs_output(
        recalc_output,
        vali_data_directory,
        dbres_output_directory,
        "models/model_Pitchs.pt",
        ...
        tmp_train = calculate_murmur_output(
        recalc_output,
        train_data_directory,
        dbres_output_directory,
        "models/model_Pitchs.pt",
        )
        tmp_test = calculate_murmur_output(
        recalc_output,
        vali_data_directory,
        dbres_output_directory,
        "models/model_Pitchs.pt",
        ...
        y_train = tmp_train.to(device)
        y_test = tmp_test.to(device)
        model_l, training_l = create_model(model_name, 3)  # 4
        training_l(
            X_train,
            y_train,
            clas_weight=weights,
    ...
    elif model_dir == "modelSystolic":
        print("Start systolic training")
        knowledge_train = torch.zeros((murmurs_train.shape[0], 2))
        for i in range(len(murmurs_train)):
            if (
                    torch.argmax(murmurs_train[i]) == 1
